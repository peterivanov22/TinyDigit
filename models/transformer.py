...
class Transformer(nn.Module):
    ...
    def calculate_latent_indices(self, T, device):
        ...
        # Number of spatial positions in the latent grid
        ...
        height_idx = pos // self.latent_height 
        width_idx = pos % self.latent_width   

        # Repeat each index num_codebooks times to account for number of codebooks
        height_idx = height_idx.repeat_interleave(self.num_codebooks)  # [0, 0, ..., 0, 1, 1, ..., 1, ..., num_codebooks-1]
        width_idx = width_idx.repeat_interleave(self.num_codebooks)    
    ...
    def forward(self, x):
        ...
        x = x + height_embeddings + width_embeddings#  (B, T, d_model)

        #vanilla positional embeddings work too
